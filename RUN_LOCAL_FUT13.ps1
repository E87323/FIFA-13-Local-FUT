...
$expectedCardsHash = '35BDAC6BB2F37F4E3F674C5BB979BCB607FEB11CD09105631796D566590FFC06'
$legacyRevivalCardsHash = '7D8A7EEAAAFEDF41EA407D497042C79E168D391D6AF5A2F692F870372284E089'
...
try {
    ...
    Write-Stage 'Matched retail CardsDLLzf.dll (CL1298564)'
    $cardsPath = Join-Path $gameDirectory 'dlc\dlc_CardsDLL\dlc\CardsDLLzf.dll'
    $packagedCards = Join-Path $root 'required\CardsDLLzf.dll'
    if (-not (Test-Path -LiteralPath $cardsPath)) { throw "Installed CardsDLL was not found: $cardsPath" }

    # The public GitHub source intentionally does not redistribute EA's DLL.
    # A local/private release may place a legally obtained matched copy in
    # required\CardsDLLzf.dll; when present the launcher validates it before it
    # is ever allowed to touch the game installation.
    $packagedCardsAvailable = Test-Path -LiteralPath $packagedCards
    if ($packagedCardsAvailable) {
        $packagedHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $packagedCards).Hash
        if ($packagedHash -ne $expectedCardsHash) {
            throw "required\CardsDLLzf.dll is present but has the wrong SHA-256. Expected $expectedCardsHash but found $packagedHash."
        }
        Write-Host 'Validated optional local matched-DLL payload.' -ForegroundColor DarkGreen
    }
    ...
    if ($installedHash -ne $expectedCardsHash) {
        if (-not $packagedCardsAvailable) {
            throw @"
Your installed CardsDLL does not match the verified CL1298564 build.
Expected SHA-256: $expectedCardsHash
Found SHA-256:    $installedHash

The public repository does not redistribute EA game binaries. Use EA App Repair/Update
so your own installation provides the CL1298564 DLL, or run IMPORT_MATCHED_CARDSDLL.cmd
and select a legally obtained matching CardsDLLzf.dll. Once required\CardsDLLzf.dll
contains the verified hash, this launcher will back up your current DLL and swap it automatically.
"@
        }
        $backupDir = Join-Path $root 'backups\game-files'
        New-Item -ItemType Directory -Force -Path $backupDir | Out-Null
        $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
        $label = if ($installedHash -eq $legacyRevivalCardsHash) { 'legacy-cl1217703-7d8' } else { 'previous' }
        $backupPath = Join-Path $backupDir ("CardsDLLzf.$label.$($installedHash.Substring(0,12)).$stamp.dll")
        Copy-Item -LiteralPath $cardsPath -Destination $backupPath -Force
        Write-Host ('Backed up current DLL: {0}' -f $backupPath) -ForegroundColor Yellow
        Copy-Item -LiteralPath $packagedCards -Destination $cardsPath -Force
        $installedHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $cardsPath).Hash
        if ($installedHash -ne $expectedCardsHash) { throw 'CardsDLL replacement failed verification.' }
        Write-Host 'Installed the matched retail 35BDAC...FFC06 CardsDLL (ProductVersion 13.0.1298564).' -ForegroundColor Green
    } else {
        Write-Host 'Matched retail CL1298564 CardsDLL is already installed.' -ForegroundColor Green
    }
    ...
    $settings.gameVersion = '13.8.1298564'
    ...
    $settings.cardsDllSha256 = $expectedCardsHash
    ...
    Write-Host 'Leave this window open. v0.14.1 uses the game-matched CL1298564 CardsDLL and keeps tracing attached after Blaze login.' -ForegroundColor White
    Write-Host 'TRACE ALWAYS ON: network + CardsDLL + screen events are recorded for the whole session.' -ForegroundColor Cyan
    ...
    while ((Get-Date) -lt $attachDeadline -and (Get-Process fifa13 -ErrorAction SilentlyContinue)) {
        if ((Test-Path -LiteralPath $autoAttachOut) -and
            ...
            Write-Host 'RE TRACE ALWAYS ON: native CardsDLL / screen / HTTP probe attached.' -ForegroundColor Green
            $attachReported = $true
            break
        }
        if ((Test-Path -LiteralPath $autoAttachErr) -and (Get-Item -LiteralPath $autoAttachErr).Length -gt 0) {
            Write-Host 'RE TRACE WARNING: autoattach reported an error. It will be included in the results ZIP.' -ForegroundColor Yellow
            break
        }
        Start-Sleep -Seconds 1
    }
    if (-not $attachReported -and (Get-Process fifa13 -ErrorAction SilentlyContinue)) {
        Write-Host 'RE TRACE: no attach confirmation yet; network/server logs are still recording.' -ForegroundColor Yellow
    }
