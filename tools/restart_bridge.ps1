...
#
# The release package includes the self-contained build outputs. Developers can
# rebuild them with:
#   dotnet publish src/bridge/Fut13Local.Bridge.csproj -c Release -o src/bridge/out
#   dotnet publish src/trace-proxy/Fut13Local.TraceProxy.csproj -c Release -o src/trace-proxy/out
...
param(
    ...
    # autoattach.ps1 reads the bridge log from here, so both scripts must agree.
    [string] $LogDirectory,
    [int]    $DimePort       = 8080,
    [int]    $Rs4Port        = 8099,
    [int]    $BlazePort      = 10092,
...
$proxyDir  = Join-Path $root 'src\trace-proxy'
...
$proxyExe = Find-BuildOutput -ProjectDirectory $proxyDir -FileName 'Fut13Local.TraceProxy.exe' `
    -BuildCommand 'dotnet publish src/trace-proxy/Fut13Local.TraceProxy.csproj -c Release -o src/trace-proxy/out'
...
if (Test-Path -LiteralPath $runtimePath) {
    ...
    foreach ($entry in @($previous.processes)) {
        ...
        if ($process.StartTime.ToUniversalTime().ToString('o') -eq $entry.startedUtc) {
...
$proxyProcess = Start-Process -FilePath $proxyExe -WorkingDirectory (Split-Path -Parent $proxyExe) `
    -RedirectStandardOutput (Join-Path $LogDirectory "proxy-$stamp.out.log") `
    -RedirectStandardError  (Join-Path $LogDirectory "proxy-$stamp.err.log") -WindowStyle Hidden -PassThru
...
$rs4Process = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $rs4Ps1, '-Port', "$Rs4Port", `
        '-FullClubCatalogPath', $rs4CatalogArg, '-ItemCatalogPath', $rs4ItemCatalogArg, `
        '-PythonPath', $rs4PythonArg, '-SavePath', $rs4SaveArg `
    -RedirectStandardOutput (Join-Path $LogDirectory "rs4-$stamp.out.log") `
    -RedirectStandardError  (Join-Path $LogDirectory "rs4-$stamp.err.log") -WindowStyle Hidden -PassThru

$dimeProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $dimePs1, '-Port', "$DimePort", '-Locale', $Locale `
...
$easwFutProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswFutPort", '-Label', 'FUT_URI' `
...
$easwAuthProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswAuthPort", '-Label', 'OSDK_EASW_AUTH_URL' `
...
$easwReqProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswReqPort", '-Label', 'OSDK_EASW_REQ_URL' `
...
$easwEventProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswEventPort", '-Label', 'OSDK_EASW_EVENT_URL' `
...
$easwMediaProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswMediaPort", '-Label', 'OSDK_EASW_MEDIA_URL' `
...
$easwGfProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswGfPort", '-Label', 'OSDK_EASW_GF_FILE_URL' `
...
$easwRosterProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $easwPs1, '-Port', "$EaswRosterPort", '-Label', 'FUT/ROSTERUPDATE_URL' `
...
$powProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $powPs1, '-Port', "$PowPort", '-Label', 'FIFA_POW_URL' `
...
$powNucleusProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $powPs1, '-Port', "$PowNucleusPort", '-Label', 'FIFA_POW_NUCLEUS_PROXY_URL' `
...
$powContentProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $powPs1, '-Port', "$PowContentPort", '-Label', 'FIFA_POW_CONTENT_SERVER_URL' `
...
$powMmmProcess = Start-Process -FilePath 'powershell.exe' `
    -ArgumentList '-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $powPs1, '-Port', "$PowMmmPort", '-Label', 'FIFA_POW_MMM_URI' `
...
$processEntries = @(
    @{ name = 'bridge'; pid = $bridgeProcess.Id; startedUtc = $bridgeProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'proxy';  pid = $proxyProcess.Id;  startedUtc = $proxyProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'rs4';    pid = $rs4Process.Id;    startedUtc = $rs4Process.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'dime';   pid = $dimeProcess.Id;   startedUtc = $dimeProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw01'; pid = $easwFutProcess.Id; startedUtc = $easwFutProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw02'; pid = $easwAuthProcess.Id; startedUtc = $easwAuthProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw03'; pid = $easwReqProcess.Id; startedUtc = $easwReqProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw04'; pid = $easwEventProcess.Id; startedUtc = $easwEventProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw05'; pid = $easwMediaProcess.Id; startedUtc = $easwMediaProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw06'; pid = $easwGfProcess.Id; startedUtc = $easwGfProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'easw07'; pid = $easwRosterProcess.Id; startedUtc = $easwRosterProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'pow12';  pid = $powProcess.Id;    startedUtc = $powProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'pow13';  pid = $powNucleusProcess.Id; startedUtc = $powNucleusProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'pow14';  pid = $powContentProcess.Id; startedUtc = $powContentProcess.StartTime.ToUniversalTime().ToString('o') }
    @{ name = 'pow15';  pid = $powMmmProcess.Id; startedUtc = $powMmmProcess.StartTime.ToUniversalTime().ToString('o') }
)
