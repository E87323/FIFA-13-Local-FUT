...
$proxy = Join-Path $root 'src\trace-proxy\out\Fut13Local.TraceProxy.exe'
if (-not (Test-Path -LiteralPath $bridge)) { throw "Bundled bridge runtime is missing: $bridge" }
if (-not (Test-Path -LiteralPath $proxy)) { throw "Bundled trace proxy is missing: $proxy" }
Write-Host 'Self-contained Blaze bridge + trace proxy: OK (no .NET SDK required for normal use).' -ForegroundColor Green
