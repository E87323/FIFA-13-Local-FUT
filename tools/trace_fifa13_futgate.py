...
function hookCardsDll() {
  ...
  send({kind: 'cardsdll-found', base: mod.base.toString(), size: mod.size,
        path: mod.path, mode: 'donor-parity-minimal'});

  // v0.13.16 Store policy: NO Store code patching.
  //
  ...
  function installChemistryRefreshHook(attempt) {
    ...
    try {
      const service = mod.base.add(0x1b0e78).readPointer();
  ...
  try {
    authListener = Interceptor.attach(mod.base.add(CARDS_RVA.AuthRequestBuild), {
      onEnter: function () {
    ...
    const payload = {
      ...
      cards_store: (function () {
        try {
          ...
          return {build: 'stock-cl1298564-35bd',
